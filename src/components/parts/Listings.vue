  ...
  <div>
    ...
    <div class="listings_wrapper">
      <div class="paginator">
        ...
        <span class="small">
          {{  (skip === 0) ? '0 - 15' : (skip > 0) ? `${skip} - ${skip + $root.$data.listings.length}` : `0 - ${skip}` }}
        </span>
        <button :class="($root.$data.listings.length < 15) ? 'small info' : 'small default'" v-on:click="handleNextPage()">
      ...
      <table class="listings">
        ...
        <tbody>
          <tr v-for="listing in $root.$data.listings" v-bind:key="listing.id.toString()" :class="listing.flash ? 'animated fadeIn' : ''">
            <td class="small_td">
            ...
            <td class="controls">
              ...
              <button class="danger" v-else-if="listing.status === 0" v-on:click="$root.cancelListing(listing)">CANCEL</button>
